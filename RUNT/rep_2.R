dataGen2 <- reactive({ 
  ...
  fechaIni <- as.Date(params()[[2]][[3]])
  fechaFin <- as.Date(params()[[2]][[4]])
  ...
  if(!is.null(params()[[2]][[6]])){
    
    initData <- initData[(initData$MARCA %in% params()[[2]][[6]]), ]
    
  } 
  
  if(!is.null(params()[[2]][[7]])){
    
    initData <- initData[(initData$DEPARTAMENTO %in% params()[[2]][[7]]), ]
    
  } 
  
  if(!is.null(params()[[2]][[8]])){
    
    initData <- initData[(initData$SERVICIO %in% params()[[2]][[8]]), ]
    
  }
  
  if (!is.null(params()[[2]][[9]])){
    
    initData <- initData[(initData$STATUS %in% params()[[2]][[9]]), ]
    
  } 
  
  if (!is.null(params()[[2]][[10]])){
    
    initData <- initData[(initData$SEGMENTO %in% params()[[2]][[10]]), ]
    
  } 
  
  if (!is.null(params()[[2]][[1]])){
    
    initData <- initData[(initData$NACIONAL_IMPORT %in% params()[[2]][[1]]), ]
...
if(input$report2 == "1. Market Share"){
  
  #Reporte de Market Share
  
  initData <- dataGen2()[[1]]
  nam <- data.frame(nombres())
  nombre <- nam$nombre[match(params()[[2]][[5]], nam$nombre2)]
  info <- initData[, which(names(dataGen2()[[1]]) %in% nombre)]
...
} else if (input$report2 == "2. Evolucion"){
  ...
  if(!is.null(params()[[2]][[campo2]])) {
    
    
    initData$newCol <- initData[, which(names(dataGen2()[[1]]) %in% nombre)]
    
    filter1 <- group_by(initData, newCol, date)
    graphData <- summarize(filter1, 
                           total = sum(CANTIDAD))
    
    graph2 <- ggplot(graphData, 
                     aes(x = date, 
                         y = total, 
                         color = newCol)) + 
      geom_line() +
...
} else if (input$report2 == "3. Contexto"){
  
  
  df <- data.frame(dataGen2()[[1]])
  ...
  } else if (input$report2 == "4. Variacion"){
