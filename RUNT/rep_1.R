...
if(input$report1 == "1. Market Share"){
  
  #Reporte de Market Share
  
  initData <- dataGen1()[[1]]
  nam <- data.frame(nombres())
  nombre <- nam$nombre[match(params()[[1]][[5]], nam$nombre2)]
  info <- initData[, which(names(dataGen1()[[1]]) %in% nombre)]
...
} else if (input$report1 == "2. Evolucion"){
  ...
  if(!is.null(params()[[1]][[campo2]])) {
    
    
    initData$newCol <- initData[, which(names(dataGen1()[[1]]) %in% nombre)]
...
} else if (input$report1 == "3. Contexto"){
  
  
  df <- data.frame(dataGen1()[[1]])
  ...
  } else if (input$report1 == "4. Variacion"){
